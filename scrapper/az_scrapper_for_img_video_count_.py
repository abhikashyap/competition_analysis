...
selectors = [
    {
        ...
        'MRP': ['[id="corePriceDisplay_desktop_feature_div" ] [class="aok-relative"]','[id="corePrice_desktop"] tbody tr .a-offscreen'],
        'Title': ['#productTitle'],
        'Rating': ['#acrPopover > span.a-declarative > a > span'],
        'Number_of_rating': ['#acrCustomerReviewText'],
        'description_data': ['#detailBulletsWrapper_feature_div','#prodDetails','[id="productDetails_feature_div"]'], 
        'Deal_of_the_day':['#dealBadgeSupportingText'],
        'Coupan' : ['.promoPriceBlockMessage > div > label'],
        'az_seller_info' : ['[id="shipsFromSoldByInsideBuyBox_feature_div"]','[class="tabular-buybox-container"]'],
...
def scrape_amazon_data_details(asin):
    html, text, curr_url = get_soup_selenium(asn=asin)
                            ...
                            if key == 'az_other_seller_info':
                                ...
                                try:
                                    if index ==0:
                                        ...
                                        for i in range(len(rows)):
                                            try:
                                                mrp_price  = rows[i].text().strip().split(':')[1].split(' ')[1]
                                                if mrp_price == '':
                                                    mrp_price = rows[i].text().strip().split(':')[1].split(' ')[-1].split('₹')[-1]
