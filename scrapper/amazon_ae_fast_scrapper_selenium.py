...
def get_soup_selenium(asn):
    ...
    url = f'https://www.amazon.ae/dp/{asn}'
    driver.get(url)
                                ...
                                for index, row in enumerate(rows):
                                    if index %2 ==0:
                                        try:
                                            ...
                                            value_text = row.css_first('div span').text().strip().replace('AED', '').replace(',','')
                                ...
                                try:
                                    if index ==0:
                                        ...
                                        for i in range(len(rows)):
                                            try:
                                                mrp_price  = rows[i].text().strip().split('AED')[-1]
                                                if mrp_price == '':
                                                    mrp_price = rows[i].text().strip().split(':')[1].split(' ')[-1].split('AED')[-1]
                                        ...
                                        rows = html.css(selector_value)
                                        if len(rows)>1:
                                            mrp_price = rows[1].text().strip().split('AED')[-1]
                                        else:
                                            mrp_price = rows[0].text().strip().split('AED')[-1]
                                        data[key] = mrp_price
                                    break
...
def amazon_scrapper(amazon_ids):
    ...
    if results:
        ...
        if 'MRP' in df.columns:
            try:
                df['MRP'] = pd.to_numeric(df['MRP'].astype(str).replace('', pd.NA).str.replace('AED', '').str.replace(',', ''), errors='coerce').fillna(0).astype(int)
            except:
                df['MRP'] = pd.to_numeric(df['MRP'].astype(str).replace('', pd.NA).str.replace('AED', '').str.replace(',', ''), errors='coerce').fillna(0).astype(float)
        
        if 'Price' in df.columns:
            try:
                df['Price'] = pd.to_numeric(df['Price'].astype(str).replace('', pd.NA).str.replace('AED', '').str.replace(',', ''), errors='coerce').fillna(0).astype(int)
            except:
                df['Price'] = pd.to_numeric(df['Price'].astype(str).replace('', pd.NA).str.replace('AED', '').str.replace(',', ''), errors='coerce').fillna(0).astype(float)
        try:
            ...
            pass
        try:
            df['price_per_unit'] = np.where(df['price_per_unit'].isna(), np.nan, df['price_per_unit'].str.replace('AED',''))
        except:
            pass
        try:
